...
startTaxID = 9606
processCount = 30 # Number of processes to be spawned for the ortholog section
...
def main():
	...
	try:
		options, arguments = getopt.getopt(sys.argv[1:],"g:e:a")
		for (opt, arg) in options:
			if (opt == '-g'):
				...
				with open("../myAPIkey") as fileIn:
					line = fileIn.readline()
					Entrez.api_key = line.rstrip()
...
def processOrthologs():
	...
	for member in result:
		if ctrIndex <= endIndex:
			chunks[ctrChunk].append(member)
		else:
			ctrChunk += 1
	...
	logFile.write('Breaking orthologs into {} chunks to control HTTP requests per second\n'.format(ctrChunk+1))
	ctrChunk = 0
	for chunk in chunks:
		jobs = []
		for x in chunk:
			p = Process(target=searchOrthologs, args=(x,))
			jobs.append(p)
			p.start()
			time.sleep(5)
		[p.join() for p in jobs]
		logFile.write('Completed chunk number:{}\n'.format(ctrChunk+1))
		ctrChunk += 1
...
def searchOrthologs(result):
	...
	try:
		...
		time.sleep(10)
		searchOrthologs(result)

	geneObject.addMember(geneMember)
	#return geneMember
...
# Return true for found files and false for no result found and accession/range/complement
	...
	if not returned and isInteger(geneValue): # No entrez accession annotation case: resolve using refseq table 
		...
		sql = 'SELECT geneID, genomic_nucleotide_accession_version, orientation, start_position_on_the_genomic_accession, '+\
			'end_position_on_the_genomic_accession FROM refseq WHERE geneID = {} AND genomic_nucleotide_accession_version IS NOT NULL'.format(geneValue)
		...
		with Entrez.efetch(db="nucleotide", id="{}".format(foundAccession), 
			...
			for record in SeqIO.parse(fastaHandle, "fasta"):
				with open(fastaDestination, 'w') as fastaOut:
					...
					# Need to change lineToWrite from a Seq object into a string
					lineToWrite = '{}'.format(lineToWrite)
...
def usage(errMessage):
	...
	with open(defaultErrorFile,'w') as err:
		err.write("Below is the error resulting from geneOrthologCollect.py on {}\n".format(time))
		err.write("{}\n".format(errMessage))
		err.write("Usage:\tgeneOrthologCollect.py -g geneID -e email@email.com")
	sys.exit(1)
