...
def main():
	...
	writeToDB(database, datfilename)

def writeToDB(database, filename):
	connection = sqlite3.connect(database)
	cursor = connection.cursor()

	sql = 'CREATE TABLE IF NOT EXISTS refseq (taxID INTEGER, geneID INTEGER, status VARCHAR(15), ' +\
...
def ftpHarvest(target, filename):
	...
	with gzip.open(target) as fileIn:
		with open(datfiledirectory + filename, 'wb') as fileOut:
